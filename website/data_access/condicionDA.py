...
class CondicionDataAccess:
    ...
    #Extracts from the database the condition by their id
    def get_condicion_by_id(self, condicion_id):
        ...
        # Initialize and return a Patient instance with the fetched data
        return Condicion(*patient)
    
    #Inserts the condition with the given inputs
    ...
    def store_visita_condicion(self, id_condicion, id_visita):
        ##Creating a connection cursor
        cursor = self.db_connection.connection.cursor()
        #Create a new VISITA_CONDICION instance for this CONDICION
        cursor.execute(''' INSERT INTO VISITA_CONDICION (id_condicion_fk, id_visita_fk) VALUES(%s, %s) ''',(id_condicion, id_visita))
        #Saving the actions performed on the DB
        db.connection.commit()
